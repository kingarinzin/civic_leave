  ...
  const fetchTeamAttendance = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/attendance/supervisor/overview?date=${selectedDate}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const filteredSubordinates = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter((officer) => {
        if (statusFilter === "late" && (officer.status === "Late arrival" || officer.status === "Late & Early"))
          return true;
        ...
        >
          <Box>
            ...
            <Text size="2" color="gray">
              Approve/reject leave requests and monitor daily attendance of your team.
            </Text>
          </Box>
        ...
        <Card size="3" mb="5" className="w-full">
          ...
          >
            <Flex gap="2" align="center" wrap="wrap">
              ...
              <Button variant="soft" size="1" onClick={exportToCSV}>
          ...
          >
            <Flex align="center" gap="2">
              ...
              <Select.Root
                value={String(rowsPerPageTeam)}
                onValueChange={(value) =>
                  setRowsPerPageTeam(value === "all" ? "all" : Number(value))
                }
              >
                <Select.Trigger />
                <Select.Content>
          ...
          <Text size="1" color="gray" mb="2">
            Showing {paginatedSubordinates.length} of {filteredSubordinates.length} entries
            {teamSearch && ` (filtered by "${teamSearch}")`}
          </Text>
                ...
                <Table.Body>
                  {paginatedSubordinates.map((officer) => (
                    <Table.Row key={officer.userId}>
                      ...
                      <Table.Cell>
                        {officer.firstIn ? (
                          <span className={officer.inColor}>{officer.firstIn}</span>
                        ) : (
                      ...
                      <Table.Cell>
                        {officer.lastOut ? (
                          <span className={officer.outColor}>{officer.lastOut}</span>
                        ) : (
                          <span className="text-gray-400">—</span>
                        )}
                      </Table.Cell>
                        ...
                        <Badge
                          color={
                            officer.status === "Present"
                              ? "green"
                              : officer.status.includes("Late") ||
                                officer.status.includes("Early")
                              ? "orange"
                              : "gray"
                          }
                          variant="soft"
                      ...
                      <Table.Cell>
                        <Button
                          ...
                          onClick={() =>
                            router.push(
                              `/dashboard/leave/attendance/history?userId=${officer.userId}&empCode=${officer.empCode}`
                            )
                          }
                        >
                          View History
        ...
        <Card size="3" className="w-full overflow-x-hidden">
            ...
            {(["all", "pending", "approved", "rejected"] as const).map((tab) => (
              ...
              >
                ...
                {tab === "pending" && counts.pending > 0 && (
                  <span className="absolute -top-2 -right-2 min-w-[18px] h-[18px] px-1.5 rounded-full bg-red-600 text-white text-xs font-bold flex items-center justify-center">
                    {counts.pending}
                  </span>
                )}
              </Button>
          ...
          >
            <Flex align="center" gap="2">
              ...
              <Select.Root
                value={String(rowsPerPageApproval)}
                onValueChange={(value) =>
                  setRowsPerPageApproval(value === "all" ? "all" : Number(value))
                }
              >
                <Select.Trigger />
                <Select.Content>
          ...
          <Text size="1" color="gray" mb="2">
            Showing {paginatedApprovals.length} of {filteredApprovals.length} entries
            {approvalSearch && ` (filtered by "${approvalSearch}")`}
          </Text>
              ...
              <Table.Root variant="surface" className="min-w-[1100px] w-full">
                ...
                <Table.Body>
                  {paginatedApprovals.map((item) => (
                    <Table.Row
                      key={item._id}
                      className={item.status !== "pending" ? "opacity-50 transition" : ""}
                    >
