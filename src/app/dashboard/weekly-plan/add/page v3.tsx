...
export default function AddWeeklyPlanPage(): React.ReactElement {
  ...
  // Track custom stakeholder text input
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      // Filter out empty tasks and legend items
      const filteredTasks = tasks.filter((t) => t.trim() !== '');
      const filteredLegend = legendItems.filter((item) => item.key.trim() !== '' && item.value.trim() !== '');
        ...
        <Flex align="center" justify="between" mb="5">
          <Box>
            ...
            <Text size="2" color="gray">
              Create a new weekly schedule with meetings, week-long tasks, and a legend.
            </Text>
          </Box>
            ...
            <Flex direction="column" gap="4">
              ...
              <div className="space-y-2">
                ...
                {legendItems.length === 0 && (
                  <Text size="2" color="gray" align="center" className="py-2">
                    No legend entries. Click "Add Legend Entry" to add one.
                  </Text>
                )}
              </div>
