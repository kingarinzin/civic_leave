// app/dashboard/weekly-plan/[id]/page.tsx
"use client";
...
export default function AdminWeeklyPlanView(): React.ReactElement {
  ...
  // =========================================================
  // 🔧 FIXED: Update meeting by _id (NOT by array index)
  // =========================================================
  const updateMeeting = (meetingId: string, field: keyof Meeting, value: string): void => {
    if (!plan) return;

    // Find the meeting by its _id and update it
  ...
  // =========================================================
  // 🔧 FIXED: Add meeting with better error handling
  // =========================================================
  const addMeeting = async (day: string): Promise<void> => {
    ...
    try {
      ...
      console.log('📤 Adding meeting to:', day);
      ...
      if (res.ok) {
        ...
        alert(`Failed to add meeting: ${error}`);
      }
    } catch (error) {
      console.error('❌ Error adding meeting:', error);
      setMessage('❌ Failed to add meeting');
      alert('Error adding meeting. Check console for details.');
    } finally {
  ...
  // =========================================================
  // 🔧 FIXED: Delete meeting by _id (NOT by array index)
  // =========================================================
  const deleteMeeting = async (meetingId: string): Promise<void> => {
    ...
    try {
      ...
      alert('Error deleting meeting. Check console for details.');
    } finally {
  ...
  return (
    <div className="flex bg-slate-50 min-h-screen">
      ...
      <main className="flex-1 p-6 ml-64">
        ...
        {/* ========================================== */}
        {/* MEETINGS GRID - WITH ADD/DELETE BUTTONS */}
        {/* ========================================== */}
        <Card size="3">
                ...
                >
                  ...
                  {dayMeetings.map((meeting) => (
                    ...
                    >
                      {isEditing ? (
                        // ==========================================
                        // EDIT MODE - All fields editable + Delete button
                        // ==========================================
                        <>
                          {/* Time + Delete Button */}
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateMeeting(meeting._id, 'location', e.target.value)
                            }
