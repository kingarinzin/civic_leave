...
export default function AdminWeeklyPlanView(): React.ReactElement {
  ...
  // ✅ FIXED: Delete meeting and force UI refresh
  const deleteMeeting = async (meetingId: string): Promise<void> => {
    ...
    try {
      ...
      if (res.ok) {
        const result = await res.json();
        console.log('✅ Meeting deleted. Remaining:', result.plan.meetings.length);
        // ✅ CRITICAL: Create new object to force re-render
        setPlan({ ...result.plan });
