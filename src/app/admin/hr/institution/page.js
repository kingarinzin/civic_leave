...
export default function InstitutionPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/hr/institution");
      if (!res.ok) {
  ...
  const handleAdd = async () => {
    if (!formData.name.trim()) {
      showNotification("Institution name is required", "error");
      return;
    }
    try {
      const res = await fetch("/api/hr/institution", {
    ...
    <div className="flex">
      ...
      <main className="flex-1 p-6 ml-64 bg-gray-100 min-h-screen">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Institution List</h1>
          {!showForm && (
            <button
              onClick={() => {
                setShowForm(true);
