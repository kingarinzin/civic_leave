...
mongoose.model("Employee", Employee.schema); // ← register Employee
...
export async function GET() {
  try {
    ...
    const logs = await TrainingLog.find()
      .populate("programTitle", "title trainingId locationCategory trainingGroup description")
      .populate("fundingAgency", "name")
      .populate("fundingModality", "name")
      .populate("institution", "name")
      .populate("employees", "fullName cidNumber positionTitle") // populate employees
      .sort({ createdAt: -1 });
...
export async function POST(request) {
  try {
    ...
    await newLog.populate([
      { path: "programTitle", select: "title trainingId locationCategory trainingGroup description" },
      { path: "fundingAgency", select: "name" },
      { path: "fundingModality", select: "name" },
      { path: "institution", select: "name" },
      { path: "employees", select: "fullName cidNumber positionTitle" },
    ]);
...
export async function PUT(request) {
  try {
    ...
    const updated = await TrainingLog.findByIdAndUpdate(_id, update, {
      ...
      .populate("fundingModality", "name")
      .populate("institution", "name")
      .populate("employees", "fullName cidNumber positionTitle");
