  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-6">
      <Card size="3" style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        {/* ========================================================= */}
        {/* PLAN DETAILS – Commented out (optional) */}
        {/* ========================================================= */}
        {/*
        <Card size="3" mb="4" style={{ background: '#f8fafc' }}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Box>
              <Text size="2" weight="medium" color="gray">Week Number</Text>
              <Text size="2">{plan.weekNumber}</Text>
            </Box>
            <Box>
              <Text size="2" weight="medium" color="gray">Year</Text>
              <Text size="2">{plan.year}</Text>
            </Box>
            <Box>
              <Text size="2" weight="medium" color="gray">Start Date</Text>
              <Text size="2">{new Date(plan.startDate).toLocaleDateString()}</Text>
            </Box>
            <Box>
              <Text size="2" weight="medium" color="gray">End Date</Text>
        ...
        */}
        ...
        {/* ========================================================= */}
        {/* ✅ MEETINGS GRID – 12-hour time & vertical stacking */}
        {/* ========================================================= */}
        <Card size="3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-7 gap-3 items-start">
            {daysToShow.map((day) => {
              ...
              // Group meetings by time slot (for the time badge)
              const grouped = dayMeetings.reduce<Record<string, Meeting[]>>((acc, meeting) => {
                const key = meeting.timeStart && meeting.timeEnd
                  ? `${formatTimeDisplay(meeting.timeStart)} - ${formatTimeDisplay(meeting.timeEnd)}`
                  : 'All Day';
                if (!acc[key]) acc[key] = [];
                      ...
                      {Object.keys(grouped).map((timeSlot) => {
                        ...
                        return (
                          <div key={timeSlot} className="border-b border-slate-100 pb-2 last:border-0">
                            ...
                            {/* ✅ Meetings in this slot – stacked vertically (not side by side) */}
